...
void request_serve_static(int fd, char *filename, int filesize) {
    int srcfd;
    char *srcp, filetype[MAXBUF], buf[MAXBUF+EXTRASPACE];
...
// handle a request: THIS PART I CHANGED A LITTLE 
/**
 * @brief handle a request 
 * 
 * @param element Node from the buffer 
 */
void request_handle(Node* element) {

    int fd = element->data; // get the file descriptor name 
    ...
    nodeDestructor(element); // don't need the node anymore 

    int is_static;
    struct stat sbuf;
    //char buf[MAXBUF], method[MAXBUF], uri[MAXBUF], version[MAXBUF]; // these were in element (the Node)
